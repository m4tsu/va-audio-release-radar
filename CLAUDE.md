...
- 定義・値・規則は 1 か所に置き、他の場所は import・生成・型で参照する。
  言語や実行環境が違って写しが避けられないときは、両者が同じ結果になることを振る舞いのテストで確かめる
- 「ここだけに置く」「写さない」「変えるときは〇〇も直す」のような注意書きをコメントや文書に書かない。
  注意書きは写しがある前提の文で、写しを防がない
- コメントの有無や文面をテストで検査しない。文面は変わるのでテストが古くなり、テストを通すための書き換えを生む
