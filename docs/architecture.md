...
決定は [`decisions/0007`](./decisions/0007-daily-crawl-from-store-feeds.md) と
[`decisions/0015`](./decisions/0015-db-is-the-ledger.md)。**cron で自動に回るのは日次と週次**で、他は手で
起動する (まだ無いものは「未実装」)。週次の対象シーズンは実行日から決め、範囲から外れた行は消さない。
...
| 月次 | 作品を持つ声優 | 新着一覧の取りこぼしを埋める。検索の上限に当たった声優の洗い出し。DLsite の既知作品の詳細を取り直して販売終了を取り下げる | GitHub Actions。ストアごとに分ける。所要は未測定 |
...
- 名寄せは完全一致と正規化一致だけ (`src/domain/identity.ts`)。複数の声優に一致したら unmatched にし
  (誤マッチより取りこぼしを選ぶ)、unmatched の credit は既存行の手動割り当てを上書きしない
- 保存する年齢区分は許可集合で決める。既定は R18 を含まず、読み取り側も R18 を除く形で絞る
- 声優に紐付かない走行 (新着一覧) では、対象声優が 1 人も解決できない作品を保存しない。声優起点の走行では保存する
- 対象声優でないと印を付けた出演者名は、再取り込みで印が消えない。未解決キューにも出ない (手動割り当てと同じ扱い)
- 作品に価格と在庫状態を持たない ([`decisions/0008`](./decisions/0008-no-price-no-availability.md))。ストアが
  販売終了を明示した listing だけ取り下げる。一覧に出ないことを理由に取り下げない
...
- **声優のデータは 3 種類に分けて持つ** (`src/server/db/schema.ts`)。**同一性** (staff id / ID / slug / 初めて見た
  日時) は作られた後に変えない。**供給元の写し** (AniList の表記・性別・画像・最後に見たシーズン) は取り込みが今回
  言った項目だけを上書きし、言わなかった項目は触らない (別の経路が埋めた値を消さないため)。**付加情報** (かな、
  表示用ローマ字) は 声優 × 属性 × 出どころ で 1 行を持ち、書き手は自分の出どころの行しか触らない
- **付加情報は読むときに、属性ごとの出どころの優先順位で 1 つ選ぶ** (`src/server/queries/actor-attributes.ts`)。
  順位は読み取り側が 1 か所に持つ。手で書いたものが先に来るので、訂正は行を足すだけで表に出て、取り込みで消えない
