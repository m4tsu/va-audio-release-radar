...
決定は [`decisions/0007`](./decisions/0007-daily-crawl-from-store-feeds.md)。**この節は決定した形で、現在は未実装**
(現在との差は「未実装」)。
...
- 対象声優が 1 人も解決できない作品は保存しない (未実装)。解決できた作品でも、検索対象の声優が credit に
  無ければその声優への credit は作らない
- 対象声優でないと印を付けた出演者名は、再取り込みで印が消えない (手動割り当てと同じ扱い。未実装)
- 作品に価格と在庫状態を持たない (未実装。[`decisions/0008`](./decisions/0008-no-price-no-availability.md))。
...
- 「データの不変条件」のうち、対象声優が 1 人も解決できない作品の除外、対象外の印、価格と在庫状態の列の削除、
  販売終了の取り下げ。現在の取り込みは検索結果の作品を全件保存し、画面に出さない価格の列を保存し続け、
  在庫状態は常に同じ値を書いている
- 検索の上限に当たった声優のページに出すストアへのリンクと、網羅率の表示
  ([`decisions/0010`](./decisions/0010-back-catalog-is-what-was-fetched.md))。網羅率は取り込みの記録にしか無い
