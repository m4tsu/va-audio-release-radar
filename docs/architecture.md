...
- 保存する年齢区分は許可集合で決める。既定は R18 を含まず、読み取り側は R18 を除く形で絞る
...
- **声優のデータは 3 種類に分けて持つ** (`src/server/db/schema.ts`)。**同一性** (staff id / ID / slug / 初めて見た
  日時) は作られた後に変えない。**供給元の写し** (AniList の表記・性別・画像・最後に見たシーズン) は取り込みが
  今回言った項目だけを上書きする。言わなかった項目は別の経路が埋めた値を消さないよう触らない。
  **付加情報** (かな、表示用ローマ字、公開状態) は 声優 × 属性 × 出どころ で 1 行を持ち、書き手は自分の
  出どころの行しか触らない。表に出す値は読み取り側が優先順位で選ぶ
