...
- **声優のデータは 3 種類に分けて持つ** (`src/server/db/schema.ts`)。**同一性** (staff id / ID / slug / 初めて見た
  ...
  表示用ローマ字、公開状態) は 声優 × 属性 × 出どころ で 1 行を持ち、書き手は自分の出どころの行しか触らない
- **付加情報は読むときに、属性ごとの出どころの優先順位で 1 つ選ぶ** (`src/server/queries/actor-attributes.ts`)。
  順位は読み取り側が 1 か所に持ち、並べていない出どころの行は選ばない。手で書いたものが先に来るので、訂正は
  行を足すだけで表に出て、取り込みで消えない。この形で読むのはかなと表示用ローマ字で、公開状態はまだ列
