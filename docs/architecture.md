...
[`decisions/0015`](./decisions/0015-db-is-the-ledger.md)。**cron で自動に回るのは日次・週次・月次**で、
残りは手で起動する (まだ無いものは「未実装」)。週次の対象シーズンは実行日から決め、範囲から外れた行は消さない。
...
| 月次 | 作品を持つ声優 | 新着一覧の取りこぼしを埋める。検索の上限に当たった声優の洗い出し。DLsite の既知作品の詳細を取り直して販売終了を取り下げる | GitHub Actions。ストアごとに分け、DLsite はさらに人数で分ける |
