...
| クローラー | Node.js (`crawler/`)。GitHub Actions の cron か手元で動く | ストアを取得して正規化し、Worker の管理 API に HTTP で送る |
...
- 検索結果に出たが対象声優の名前が credit に無い作品は、保存はするがその声優への credit を作らない
...
通知 (配信手段、ログイン時のフォロー同期)。本番デプロイと日次クロールの運用開始。
