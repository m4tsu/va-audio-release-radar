...
- 方針: 検索 1 ページ目 (新しい順) で ID を集め、埋め込み JSON の `pager.count` が実取得件数を上回るときだけ `order/release` (古い順) の 1 ページ目を足して和集合にする (最大 60 件)。DB に無い ID だけ `product.json` を 1 件ずつ取る。**リクエスト間隔は検索も product.json も 10 秒** (robots.txt の `Crawl-delay: 10` は `User-agent: *` グループにあり全パスに及ぶ。§13。以前ここに書いていた「product.json 2 秒」は誤り)
...
- robots.txt: `searchNarrator=` に `page=` や `node=` を組み合わせたものは不許可。1 ページ目 (20 件) だけ取る制約は残る。総件数は「検索結果 N のうち…」の表示から取り、20 を超えるときだけ `sort=pubdate-asc-rank` (古い順) の 1 ページ目を足して和集合にする (最大 40 件)。それでも総件数に届かなければ adapter が網羅率を警告に積む (企画書 §21)
...
## 13. 差別化の柱と取得方針 (2026-09-18 決定)

外部レビュー (`docs/research/codex-differentiation-spec.md`) の 3 案を実データ (90 日 428 作品) で検証した結果。

| 柱 | 実データ | 決定 |
|---|---|---|
| A 共演バッジ (推し 2 人出演) | AniList 声優が 2 人以上の作品は 75 件中 16 件 (21%)。中身は「同じ企画への複数出演」で、有名声優同士の偶然の共演ではない | **低優先**。クレジットは全員分保存済みなので、集合積だけでいつでも実装できる。データ基盤の追加対応は不要 |
| B 出演形態 (単独 / 少人数 / 全編朗読 / 大人数) | DLsite は 61% が単独、8% が 2 人。`voice_by` 配列長と Audible のナレーター配列長で判定でき、実装コストはほぼゼロ | **採用。差別化の軸に置く**。価値は Audible 側で特に高い (全編朗読か 20 人中 1 人かで購買判断が変わる)。来訪頻度に依存せず back catalog 中心の設計とかみ合う |
| C 商業イベント (セール / 予約 / 無料) | **セール中が 41%** で常態。予約作品は全年齢音声で 0 件。無料は 0.6% | **スキップ**。セールは通知ではなく声優ページのフィルタとして持つ。「新規にセールになった」の検出自体は既存スキーマ (price / list_price) の前回値比較で可能だが、back catalog を毎日引く必要がありコストが見合わない。後から始めても失うデータはない |

外部レビューはフィードを濃くする方向を提案しているが、本プロジェクトは実測から「フィードは薄いのが前提、back catalog が主内容」(§12) と結論している。実測値のある側を採る。ただし同レビュー §16 の判断規則 (「この機能はフォロー情報があるから成立するか」) と §20 の原則は採用する。

### 取得方針: 声優起点。全件取得はしない

- robots.txt の `Allow: /*/fsr/=/*/per_page/*/page/1/` により、**`per_page` 付きでも 1 ページ目は許可**されている (従来「per_page は禁止」と誤読していた)
- 対象声優を「AniList にアニメ出演がある人」(§9) と定義した時点で、DLsite 側から声優を発見する必要はない。声優名で引けばよい
- コスト比較 (10 秒間隔): 声優起点 2,569 人 = 7.1 時間 / sitemap 全件 68,321 作品 = 190 時間。**27 倍の差**
- sitemap による全件発見は「AniList に居ない声優の発見」にしか要らず、それは対象外
- `Crawl-delay: 10` は `User-agent: *` グループにあり全パスに及ぶ。`product.json` の 2 秒間隔は 10 秒に揃える

### 取得上限の実測 (2026-09-18、A4 調査)

- **`per_page` は無視される**。位置を変えても値を 10 に下げても既定件数が返る。過去の「0 件」は再現せず、無視されるのが正しい挙動 (§3 の記述を訂正)
- **総件数は埋め込み JSON `"pager":{"count":N,"have_to_paginate":bool}` から必ず取れる**。これを保存すれば網羅率を監視できる
- 上田麗奈の全年齢音声作品は**全期間で 27 件** (`pager.count` と実取得件数が一致)。既存の 1 ページ目取得で 100% 網羅していた。既知の声優で最多 27 件、30 件超はゼロ
- 「90 日制限」は企画書の記述であって実装上の制約ではなかった。新しい順 1 ページが大半の声優にとって全期間そのもの (§12 の「撤廃方向」という課題はほぼ存在しなかった)
- 30 件を超えうるのは 245 名義中 4 名義 (2%) で、いずれも同人 ASMR 専業で AniList に居ない = 対象外。対象に入る層の最多は富田美憂の年換算 24 件
- 対応方針: `pager.count` を取得して `crawl_runs` に記録し、実取得件数が下回った場合だけ `order/release` (古い順) を足して 60 件まで補完する
- 検索結果 HTML から現在価格・定価・**割引率バッジ** (`20%OFF`) まで取れる。セール終了日時だけは `product.json` の `campaign_end_date` が必要
- 参考 (セール検出を後から始める場合の規模): 新作割引を除く値引き中は 21%、セール期間の中央値 13 日。フォロー 10 人・200 作品なら 1 日 3 件程度

## 14. 企画書の位置づけと年齢区分の方針 (2026-09-18 決定)

### 企画書は草案

`docs/development-plan.md` は最初の草案であり、厳守すべき方針ではない。企画書は「小さい MVP を作る」ことを正義としがちだが、それは本プロジェクトが目指す方針ではない。実装と議論で得た事実が企画書と食い違う場合、**事実と本設計書を優先する**。企画書の記述を根拠に選択肢を狭めない。

これまでに企画書から変更した主な点: 対象声優の決め方 (§9)、新着の定義と 90 日制限 (§10, §12)、差別化の柱 (§13)、フィードの薄さを kill criteria から外したこと (§12)。

### 年齢区分

- **「全年齢のみ」という制限は設けない** (企画書 §26 の推奨を採らない)
- ストアごとに実利が違う:
  - **ポケドラの BL / R18 は実利が大きい**。BL ドラマ CD はアニメ声優が本名義で出演するのが普通で、斉藤壮馬は 143 件中 98 件が BL。除外すると対象の 7 割を失う
  - **DLsite maniax (R18) の実利は未測定**。R18 音声の多くは専業の同人声優かアニメ声優の別名義で、本名義での出演がどれだけあるかは測っていない
- **別名義は「信頼できる情報に基づく場合に限り」紐付ける** (2026-09-18 ユーザー決定)。紐付けられるならプラスだが、根拠が要る。成人向けかどうかとは独立した原則
  - 根拠として認めるもの: 本人・事務所が公表している情報、ストア自身が同一人物として扱っている事実、出典のある公開情報
  - **DLsite からは別名義の根拠が得られないことを実測で確認した** (2026-09-18)。スナップショット 2,275 件で `creaters` の id と name は完全な 1 対 1 で、同じ id に複数名義の例は 0 件。1 つの id が複数の役割 (voice_by と music_by) にまたがる例は 138 件あり、id が人単位の識別子であることは確かだが、**名義はまたがない**。声優個人のプロフィールページも存在せず (プロフィールがあるのはサークルのみ)、UI 上クリエイターは名義で串刺しされていて人では串刺しされていない
  - したがって当面、別名義の根拠は本人・事務所の公表情報に限られる。R18 を対象外とした以上 (下記) 優先度は低い
  - 認めないもの: 声質・作風・活動時期などからの推測、LLM による判断、「たぶん同じ人」という運用者の心証
  - 実装は既存の `VoiceActorAlias` の `verified` / `source` で表現する。`verified: true` は根拠を確認したものだけに付け、`source` に由来を残す
  - 間違えたときの被害 (実在の人物に、公表していない出演作を結び付ける) が大きいため、迷ったら紐付けない
- 年齢区分を扱うために別途必要になるもの: 年齢確認の導線、アフィリエイト規約の確認 (DLsite / Amazon アソシエイトでアダルト商品の扱いが異なる)、ホスティング規約の確認

### 年齢区分の実測と結論 (2026-09-18、A6 調査)

`docs/research/adult-scope-2026-09-18.md` で実測した結果、**R18 を載せる実利はほぼ無く、リスクだけがある**ことが分かった。
...
| ポケドラ BL (全年齢) | 911 作品。年齢確認も Cookie も不要、声優は**本名義**でタグ付き。斉藤壮馬は BL だけで 98 件 |
...
**決定**: 対象は**全年齢 + BL (全年齢 BL)**。R18 は載せない。これは「全年齢のみ」という §26 の制限に戻ることではなく、次の事実に基づく判断である。

- R18 を載せても対象声優 (アニメ声優の本名義) の作品は 1 件も増えない
- 一方で Amazon アソシエイトの審査に落ちるリスクがある
- BL を含めることが重要な変更であり、これを外すと斉藤壮馬のような男性声優の対象作品を 7 割失う

将来 R18 を扱う判断をする場合も、別名義の紐付けは §14 の原則 (信頼できる情報に基づく場合のみ) に従う。

**設計の修正が必要な箇所**:
- `AudioWork.adult: boolean` では BL と一般を区別できない。**年齢区分は真偽値ではなく列挙**にする (`general` / `bl` / `r18` / `r18_bl` 相当)。ポケドラのストア区分 (`men` / `bl` / `adt` / `adt-bl`) がそのまま取れる
- §1 の「導入しないもの: …成人向け作品」の記述は本節で置き換える
- ポケドラは 1 声優あたり 2 リクエスト (一般 + BL)。オトナ向け 2 ストアを外せば Cookie もセッション維持も不要でクローラーが単純なまま
