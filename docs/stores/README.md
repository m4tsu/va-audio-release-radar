...
| ストア | ファイル | 状態 |
|---|---|---|
| DLsite | [`dlsite.md`](./dlsite.md) | 全年齢の `/home/` と女性向け全年齢の `/garumani/` の 2 フロアを引く。R18 を含むフロアは引かない |
...
**UA は相手によって要求が逆になる。** スクレイパー判定で 302 に飛ばす相手
([`audible.md`](./audible.md)) にはブラウザ相当の UA を、ブラウザの UA を名乗ることを禁じている相手
([`wikimedia.md`](./wikimedia.md) の「既知の落とし穴」) には連絡先つきの自己申告 UA を送る。
振り分けは `fetch.ts` の `userAgentFor()` が持つ。新しい相手を足す前に、その相手の UA の要求を読む。
