...
収まらないフロアでは古い順の 1 ページ目も引く。`/home/` ではそれが 2% の名義にしか起きない
のに対し、`/garumani/` は 30 件の上限に当たる声優が珍しくない (下の「既知の落とし穴」)。
...
- **`/home/` では 1 声優あたりの作品数が少ない。** 上田麗奈の全年齢音声作品は全期間で 27 件で、
  ...
  **`/garumani/` はこの限りではない。** 斉藤壮馬は 52 件で、1 ページ 30 件の上限に当たっている
  (2026-09-20 実測。上の「既知の落とし穴」と同じ出典)
