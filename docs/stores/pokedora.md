...
| 項目 | 可否 | セレクタ / 備考 |
|---|:--:|---|
| 総件数 | ○ | `div.search_count` の `1,365件中　1 - 15件目` の形 |
| ページ番号 | ○ | ページャの `a[onclick]` の `$('#p_pageno').val(91)` の形。`href` は持たない |
...
- **`list.php` では `disp_number` が効かない。** 1 ページ 15 件で固定される。
  30 / 50 / 100 の `select` を持っているのは `/tags/` だけで、`list.php` のフォームには無い。
  総件数 1,365 件に対して最終ページが 91 (= ceil(1365 ÷ 15)) であることとも一致する
...
- **「NEW」バッジが何日間付くか。** 一般 21 件 / BL 3 件という件数を日数に直せない
- **1 ページ目 (15 件) に出ない新商品があるか。** 15 件より深いページは一般の 2 ページ目しか引いていない
- **一覧から商品が消える条件。** 3 日で 一般 が 1 件減っており、非公開化か別ストアへの移動かは見ていない
