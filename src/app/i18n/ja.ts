/**
 * 日本語の辞書。`en.ts` は、このファイルから作る `Translations` 型に合わせて書く。
 *
...
export const ja = {
  ...
  push: {
    ...
    privacyLink: "プライバシーポリシー",
    /** 見出しの隣のヘルプの印の読み上げ名。説明と保存の注記はその中に出す */
    helpLabel: "新作の通知の説明",
    /** 声優ページでフォローした直後に出す案内。通知の設定がフォロー中ページにしか無いことを補う */
    afterFollow: "フォローした声優の新作が出たら、このブラウザに通知できます。",
  },
  ...
  following: {
    ...
    /** 見出しの隣のヘルプの印の読み上げ名。保存先の注記はその中に出す */
    storageHelpLabel: "フォローの保存先の説明",
  ...
  actor: {
    ...
    /**
     * 一覧を上限で切ったとき。並べた件数は絞り込みの横の件数で読めるので、
     * 並び順だけを言い、新しい方から載せていることを伝える
     */
    listLimited: "新着順",
    ...
    /** 取得する範囲も並べる件数も直近のシーズンに限っているので、全出演歴と読ませない */
    animeTitle: "直近出演アニメ",
    /** 作品一覧が長いと出演アニメが視界に入らない。見出しの下から飛ぶリンク */
    jumpToAnime: "直近出演アニメへ",
  },
