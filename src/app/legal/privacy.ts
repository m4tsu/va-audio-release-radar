 ...
 * - 操作の計測: `src/app/lib/usage-events.ts` (操作の種類とストアの別を `/api/event` へ送る。
        ...
        blocks: [
          {
            ...
            items: [
              ...
              "声優ページでフォローを押したときと、作品ページで販売サイトへのリンク (Audible の無料体験への登録を含む) を押したとき、その操作の種類と、リンク先の販売サイトの別がサーバーに送られ、回数を集計するために Cloudflare, Inc. の Workers Analytics Engine に保存されます。この記録には、フォローした声優、作品、IP アドレスなど、操作した人や操作の対象を特定しうる情報を含めません。フォローを外したときは送られません。",
        ...
        blocks: [
          {
            ...
            items: [
              ...
              "When you press Follow on a voice actor's page, or a link to a store on a work page (including the link to sign up for the Audible free trial), the kind of action and which store the link leads to are sent to the server and stored in Workers Analytics Engine, operated by Cloudflare, Inc., to count how often each happens. This record does not include the voice actor you followed, the work, your IP address or anything else that could identify you or what you acted on. Nothing is sent when you unfollow.",
