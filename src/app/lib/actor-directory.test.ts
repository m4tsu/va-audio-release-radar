...
const ACTORS: Row[] = [
  { slug: "a", workCount: 1, storeSlugs: ["dlsite"], gender: "unknown" },
  { slug: "b", workCount: 5, storeSlugs: ["audible", "pokedora"], gender: "unknown" },
  { slug: "c", workCount: 5, storeSlugs: ["dlsite", "audible"], gender: "unknown" },
];

/** 英語表記つき。AniList の fullName と同じく名が先に来る */
const EN_ACTORS: Row[] = [
  { slug: "ueda", workCount: 1, storeSlugs: ["dlsite"], nameEn: "Reina Ueda", gender: "unknown" },
  {
    slug: "hikasa",
    workCount: 2,
    storeSlugs: ["audible"],
  ...
  { slug: "yukana", workCount: 3, storeSlugs: ["dlsite"], nameEn: "Yukana", gender: "unknown" },
  { slug: "noname", workCount: 4, storeSlugs: ["dlsite"], gender: "unknown" },
];
...
describe("arrangeActors", () => {
  ...
  test("作品が 1 件も無い声優は作品数の多い順で末尾に来る", () => {
    const withEmpty: Row[] = [
      { slug: "empty", workCount: 0, storeSlugs: [], gender: "unknown" },
      ...ACTORS,
    ];

    expect(
      slugs(
        arrangeActors(withEmpty, {
          sort: "workCount",
          store: null,
          gender: "all",
          initial: null,
  ...
  test("絞り込んだ結果が 0 人になることもある", () => {
    const onlyDlsite: Row[] = [
      { slug: "a", workCount: 1, storeSlugs: ["dlsite"], gender: "unknown" },
    ];
    expect(
      arrangeActors(onlyDlsite, {
        sort: "name",
        store: "audible",
        gender: "all",
        initial: null,
...
describe("性別の絞り込み", () => {
  ...
  const GENDERED: Row[] = [
    ...
    { slug: "o", workCount: 1, storeSlugs: ["audible"], gender: "other" },
    { slug: "u", workCount: 1, storeSlugs: ["audible"], gender: "unknown" },
  ];

  const filtered = (gender: ActorGenderFilter, store: StoreSlug | null = null) =>
    slugs(arrangeActors(GENDERED, { sort: "name", store, gender, initial: null, locale: "ja" }));

  test("女性を選ぶと女性だけが並ぶ", () => {
    expect(filtered("female")).toEqual(["f"]);
  });
  ...
  /** 押せる頭文字を数える側も同じ絞り込みを通らないと、押した結果が 0 人になる文字が出る */
  test("押せる頭文字も性別で絞った後の顔ぶれから決まる", () => {
    const rows: Row[] = [
      {
        slug: "ueda",
        workCount: 1,
      ...
      { slug: "kaji", workCount: 1, storeSlugs: ["dlsite"], nameEn: "Yuki Kaji", gender: "male" },
    ];
    expect(availableInitials(rows, { store: null, gender: "female", locale: "en" })).toEqual(["U"]);
  });
});

describe("availableInitials", () => {
  test("英語表示ではローマ字の姓の頭文字をアルファベット順に返す", () => {
    expect(availableInitials(EN_ACTORS, { store: null, gender: "all", locale: "en" })).toEqual([
      "H",
      "U",
      "Y",
    ]);
  ...
  test("日本語表示では何も返さない", () => {
    expect(availableInitials(EN_ACTORS, { store: null, gender: "all", locale: "ja" })).toEqual([]);
  });

  /** 押した結果が 0 人になる文字は出さない */
  test("ストアで絞った後に居ない文字は返さない", () => {
    expect(availableInitials(EN_ACTORS, { store: "audible", gender: "all", locale: "en" })).toEqual(
      ["H"],
    );
  });
  ...
  test("同じ頭文字は 1 つにまとまる", () => {
    const rows: Row[] = [
      { slug: "u1", workCount: 1, storeSlugs: ["dlsite"], nameEn: "Reina Ueda", gender: "unknown" },
      { slug: "u2", workCount: 1, storeSlugs: ["dlsite"], nameEn: "Kana Ueno", gender: "unknown" },
    ];
    expect(availableInitials(rows, { store: null, gender: "all", locale: "en" })).toEqual(["U"]);
  });
});

describe("actorInitial", () => {
  test("姓の頭文字を大文字で返す", () => {
    expect(actorInitial({ nameEn: "Reina Ueda" }, "en")).toBe("U");
  });

  test("1 語の名義はその語の頭文字", () => {
    expect(actorInitial({ nameEn: "Yukana" }, "en")).toBe("Y");
  });

  test("ローマ字が無ければ頭文字も無い", () => {
    expect(actorInitial({}, "en")).toBeNull();
  });
  ...
  test("空白しか無い表記は頭文字を持たない", () => {
    expect(actorInitial({ nameEn: " " }, "en")).toBeNull();
  });
  ...
  test("日本語表示では頭文字を持たない", () => {
    expect(actorInitial({ nameEn: "Reina Ueda" }, "ja")).toBeNull();
  });
});
